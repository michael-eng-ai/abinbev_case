...
# JAVA_HOME - usa variavel de ambiente ou detecta automaticamente
...
with DAG(
    ...
    bronze_ingestion = BashOperator(
        ...
        bash_command=f"{JAVA_HOME_EXPORT}poetry run python {NOTEBOOKS_PATH}/01_bronze_ingestion.py",
    ...
    silver_transformation = BashOperator(
        ...
        bash_command=f"{JAVA_HOME_EXPORT}poetry run python {NOTEBOOKS_PATH}/02_silver_transformation.py",
        cwd=str(PROJECT_ROOT),
    )
    ...
    gold_business_rules = BashOperator(
        ...
        bash_command=f"{JAVA_HOME_EXPORT}poetry run python {NOTEBOOKS_PATH}/03_gold_business_rules.py",
        cwd=str(PROJECT_ROOT),
    )
    ...
    consumption_dimensional = BashOperator(
        ...
        bash_command=f"{JAVA_HOME_EXPORT}poetry run python {NOTEBOOKS_PATH}/04_consumption_dimensional.py",
        cwd=str(PROJECT_ROOT),
    )
