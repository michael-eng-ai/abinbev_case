...
class OpenMetadataClient:
    ...
    def register_lineage(self, edge: LineageEdge) -> Dict:
        ...
        payload = {
            "edge": {
                "fromEntity": {"type": "table", "fqn": edge.source_table},
                "toEntity": {"type": "table", "fqn": edge.target_table},
                "lineageDetails": {
    ...
    def register_data_quality(self, result: DataQualityResult) -> Dict:
        ...
        payload = {
            "testCaseResult": {
                ...
                "testResultValue": [{"name": result.test_name, "value": str(result.value)}],
            }
        }
    ...
    def update_table_description(self, table_fqn: str, description: str) -> Dict:
        ...
        payload = [{"op": "add", "path": "/description", "value": description}]
    ...
    def add_tag(self, table_fqn: str, tag: str) -> Dict:
        ...
        payload = [{"op": "add", "path": "/tags/0", "value": {"tagFQN": tag}}]
...
class DataQualityReporter:
    ...
    def report_check(
        self, test_name: str, test_type: str, passed: bool, value: float, threshold: float
    ):
...
def register_pipeline_lineage(
    source_layer: str, target_layer: str, transformations: List[Dict[str, str]]
):
    """
    ...
    for t in transformations:
        tracker.add_table_transformation(t["source"], t["target"], t.get("desc", "Transformation"))
...
def report_dq_results(table_name: str, checks: List[Dict[str, Any]]):
    ...
    for check in checks:
        reporter.report_check(
            check["name"], check["type"], check["passed"], check["value"], check["threshold"]
        )
